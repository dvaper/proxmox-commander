...
class VMDeploymentService:
    ...
    def generate_tf_content(
        ...
        template_line = f"template_id   = {template_id}" if template_id else "template_id   = var.default_template_id"
...
'''
    ...
    async def validate_config(self, config: VMConfigCreate) -> VMValidationResult:
        ...
        # Prüfe Node
        if config.target_node.value not in PROXMOX_NODES:
            errors.append(f"Unbekannter Proxmox-Node: {config.target_node}")
        else:
            node_info = PROXMOX_NODES[config.target_node.value]
            if config.cores > node_info["cpus"]:
                warnings.append(
                    f"Angeforderte Kerne ({config.cores}) überschreiten "
                    f"Node-Kapazität ({node_info['cpus']})"
                )
            if config.memory_gb > node_info["ram_gb"]:
                warnings.append(
                    f"Angeforderter RAM ({config.memory_gb} GB) überschreitet "
                    f"Node-Kapazität ({node_info['ram_gb']} GB)"
                )
    ...
    async def preview_config(self, config: VMConfigCreate) -> TerraformPreview:
        ...
        content = self.generate_tf_content(
            ...
            vmid=vmid,
            ip_address=ip_address,
            target_node=config.target_node.value,
    ...
    async def destroy_vm(self, name: str, user_id: int) -> int:
        ...
        # Callback für IP-Freigabe und Ansible-Inventory-Update bei erfolgreichem Destroy
        async def on_destroy_success():
            """Gibt die IP in NetBox frei und entfernt VM aus Ansible-Inventory"""
            # 1. IP in NetBox freigeben
            await netbox_service.release_ip(vm_config.ip_address)

            # 2. VM aus Ansible-Inventory entfernen
            try:
                ansible_inventory_service.remove_host(name)
            except Exception as e:
                # Inventory-Fehler loggen, aber Destroy als erfolgreich werten
                print(f"Warnung: Ansible-Inventory-Update fehlgeschlagen: {e}")
    ...
    def _parse_tf_file(self, content: str) -> Optional[dict]:
        ...
        try:
            ...
            ansible_group_match = re.search(r'# Ansible-Gruppe:\s*(\S*)', content)
    ...
    async def get_unmanaged_vms(self) -> list[dict]:
        ...
        Returns:
            Liste von VMs mit vmid, name, node, status, cores, memory, disk
        """
    ...
    async def import_existing_vm(
        ...
        for key in ["ipconfig0", "ipconfig1"]:
            if key in config:
                ipconfig = config[key]
                # Format: ip=192.168.60.100/24,gw=192.168.60.1
        ...
        if not ip_address:
            return {"success": False, "error": "IP-Adresse konnte nicht aus VM-Konfiguration extrahiert werden"}

        # VLAN aus IP extrahieren
        vlan = int(ip_address.split(".")[2])

        # Ressourcen aus Konfiguration
        ...
        content = self.generate_tf_content(
            ...
            ip_address=ip_address,
            target_node=node,
            description=config.get("description", f"Importiert aus Proxmox"),
            cores=cores,
            memory_gb=memory_gb,
            disk_size_gb=disk_size_gb,
            vlan=vlan,
        ...
        # Optional: IP in NetBox registrieren
        if register_netbox:
            try:
                await netbox_service.reserve_ip(
                    ip_address=ip_address,
                    description=f"VM: {vm_name} (importiert)",
                    dns_name=f"{vm_name}.newsxc.net",
                )
