...
async function loadData() {
  ...
  // Verfügbare Gruppen und Playbooks laden (für Super-Admin: alle)
  try {
    const [groupsRes, playbooksRes] = await Promise.all([
...
async function saveUser() {
  ...
  try {
    if (editMode.value) {
      ...
      // Gruppen und Playbooks aktualisieren (nur wenn nicht Super-Admin)
      ...
      // Gruppen und Playbooks setzen (nur wenn nicht Super-Admin)
      if (!editData.value.is_super_admin && (selectedGroups.value.length || selectedPlaybooks.value.length)) {
